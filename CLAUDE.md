...
- **차트의 accent는 '선택 상태'다.** 지금 걸린 필터가 어느 구간을 잡았는지 보여준다.
  칩을 지우면 그 차원의 강조가 사라진다. accent 용법 3가지 안에 있다
- 강조된 구간의 합은 KPI와 같다 (진행 중 128 = PTSD 128 = 여 128 = 40대 이상 68+38+22)
...
- 성별만 분포가 아니라 **비율**이다. 둘이 합쳐 트랙을 채우고 여성분이 오른쪽에서 자란다
- 집계 숫자는 백엔드가 내려줄 값이다. **칩을 지워도 숫자는 다시 계산되지 않는다** — 확인 필요
