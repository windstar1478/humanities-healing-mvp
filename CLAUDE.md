...
- 홈(업무) 화면: 빠른 저작 · 미배정 · 일정 타임라인 7일치(7/26~8/1) + 날짜 이동
- 우측 환자 패널(셸 전역)
- 꾹 누르기 → 드롭 상태 → 고스트 → 확인 모달 → 배치. **환자와 할 일 모두**,
  **아젠다와 캘린더 모두**에서 동작
- 모달 하나로 통합(배치 확인 · 일정 추가 · 할 일 추가) + 중복 경고 + 날짜/시간 선택
- 일정 화면: 2026년 7월 월 그리드 + 날짜 팝오버 + 날짜 드롭 + 빈 칸 탭 → 일정 추가
- 모달 환자 리스트 2구간(최근 환자 / 전체 환자), 업무 날짜·시간 '미정' → 미배정 할 일

미구현: 일정 편집·삭제, 팝오버의 chevron(환자 상세), 환자 분석 · 효과성 분석 · 저작도구 화면

**되돌리기 경로가 아직 없다.** 배치는 되지만 지우거나 고칠 수 없다.
...
- `src/scheduleState.js` — **일정의 유일한 가변 상태.** 화면이 사본을 들면
  ...
  **미배정 할 일(`taskState`)도 여기 있다.** 캘린더에서 만든 미정 업무가
  홈에 나타나야 하므로 화면 로컬 상태로 둘 수 없다
...
앞선 4건(모달 환자 리스트 2구간 · 업무 미정 · 캘린더 빈 칸 · 드롭 중 인접 달)은
모두 반영했다. 규칙은 위 '확정 설계 규칙'과 '현재 상태'에 흡수되어 있다.

남은 것 중 이번에 건드리지 않은 부분:
- **미배정 할 일을 배치해도 목록에서 사라지지 않는다.** 되돌리기 경로(일정 편집·삭제)를
  만들 때 같이 정할 것 — 배치가 되돌려지면 할 일도 돌아와야 한다
- **'할 일 추가'에는 날짜 선택이 없다.** 미정 업무는 '일정 추가 → 업무 → 미정'으로만
  날짜를 남길 수 있다. 두 경로를 합칠지는 미정

다음: **환자 분석** 화면 (좌측 네비 3번째). Figma 노드는 아래 목록 참고.
