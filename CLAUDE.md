...
미구현: 일정 편집·삭제, 환자 상세 화면(아젠다 환자 블록 · 팝오버 chevron이 여기로 간다),
환자 분석 · 효과성 분석 · 저작도구 화면
...
**환자 분석 화면**(좌측 네비 3번째)이 우선이다. Figma 노드는 아래 목록 참고.

Figma는 `.mcp.json`의 원격 서버(`https://mcp.figma.com/mcp`)로 읽는다.
붙어 있지 않으면 노드 값을 확인할 방법이 없으니 **추측해서 채우지 말고 먼저 알릴 것.**

그 뒤로 환자 상세 → 일정 편집·삭제(되돌리기 경로) → 코어 프로세스 6단계.
지금 아젠다의 환자 블록과 팝오버 chevron은 갈 곳이 없어 탭해도 아무 일도 일어나지 않는다.
...
- `87:763` / `127:8671` / `130:2792` — 환자 관련 화면
