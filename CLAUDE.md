...
- 일정 화면: 2026년 7월 월 그리드 + 날짜 팝오버 + 날짜 드롭
...
아래 4건은 사용자가 지적했고 아직 손대지 않았다. 원인까지 파악된 상태다.

**1. 모달 환자 리스트를 우측 패널과 같은 구조로**
지금은 `최근` 라벨 + 리스트 하나뿐이다. 우측 환자 패널처럼 `최근 환자` 다음에
`전체 환자`가 이어지고 스크롤로 내려가는 형태로 바꿀 것.
현재 코드: `ScheduleEntryModal.vue`의 `patientResults` — 검색어가 없으면
`recentPatients`만 반환한다. 두 구간을 모두 렌더하도록 바꿔야 한다.

**2. 업무(work)는 날짜·시간 미지정을 허용**
환자 일정과 달리 업무는 언제 할지 안 정한 상태로 만들 수 있어야 한다.
⚠️ 설계 충돌 주의: 아젠다 타임라인은 시간 키(`HOURS`) 기준이라 시간이 없는 이벤트는
그릴 자리가 없다. 미지정 업무를 **미배정 할 일 목록으로 보낼지**, 아니면 타임라인에
별도 구간을 둘지 먼저 정해야 한다. 전자가 기존 구조와 맞아 보인다.

**3. 캘린더 빈 칸 터치 → 바로 일정 추가**
지금은 `ScheduleView.vue`의 `openDay()`가 `if (!cell.events.length) return`으로
빈 칸을 막고 있다. 빈 칸을 누르면 그 날짜로 일정 추가 모달이 열려야 한다.
**"일정이 없는 칸은 열리지 않는다"는 기존 규칙을 이걸로 교체한다.**

**4. 드롭 중 인접 달 칸이 구분되지 않음**
앞뒤 달 칸은 평소 `opacity-40`인데, 드롭 상태의 배경색까지 40%로 씻겨서
8월 칸에 놓으려 할 때 어디가 대상인지 보이지 않는다.
드래그 중에는 `dimmed`의 `opacity-40`을 적용하지 않는 방향으로 고칠 것.
(`ScheduleView.vue` 셀의 `:class` 배열에서 `cell.dimmed`와 `dropState`가 겹친다)

그 다음: **환자 분석** 화면 (좌측 네비 3번째). Figma 노드는 아래 목록 참고.
